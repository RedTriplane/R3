...
public final class Box2DWorldWrap {
	...
	public Box2DWorldWrap (final Box2DWorldConfig box2d_world_config, final RedTriplanePhysics redTriplanePhysics) {
		...
		final Box2DWorldSpecs specs = redTriplanePhysics.box2d.newBox2DWorldSpecs();

		specs.setGravity(gravity_vector);
		specs.setDoSleep(doSleep);

		this.gdx_box2d_world = redTriplanePhysics.box2d.newBox2DWorld(specs);
		this.positionIterations = box2d_world_config.getPositionIterations();
		this.velocityIterations = box2d_world_config.getVelocityIterations();
		this.box2d_time_step_delta = box2d_world_config.getBox2DTimeStepping();
		this.gdx_box2d_world.setContactListener(this.contactListener);
		this.gdx_box2d_world.setAutoClearForces(box2d_world_config.getAutoclearForces());
	...
	public Box2DComponent getBox2DComponent () {
		return this.redTriplanePhysics.box2d;
	}

	final public void dispose () {
		this.gdx_box2d_world.dispose();
	}
