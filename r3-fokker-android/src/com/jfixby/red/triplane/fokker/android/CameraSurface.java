...
// Camera.Size previewSize = previewSizes.getElementAt(0);
		{
			...
			if (display.getRotation() == RedDisplay.ROTATION.ROTATION_0) {
				parameters.setPreviewSize(height, width);
				this.camera.setDisplayOrientation(90);
			}

			if (display.getRotation() == RedDisplay.ROTATION.ROTATION_90) {
				parameters.setPreviewSize(width, height);
			}

			if (display.getRotation() == RedDisplay.ROTATION.ROTATION_180) {
				parameters.setPreviewSize(height, width);
			}

			if (display.getRotation() == RedDisplay.ROTATION.ROTATION_270) {
				parameters.setPreviewSize(width, height);
...
// parameters.setPreviewSize(previewSize.height, previewSize.width);
	...
	private RedDisplay getDefaultDisplay () {
		...
		final RedDisplay display = new RedDisplay(shitDisplay);
		return display;
	}
