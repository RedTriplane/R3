...
public class JPoint2D implements Float2 {
	final private Vector2 gdx_vector;
	private final Float2 redPoint;

	public JPoint2D (final Vector2 position) {
	...
	@Override
	public JPoint2D setXY (final double x, final double y) {
		this.redPoint.setXY(x, y);
		return this.update();
	}

	@Override
	public JPoint2D setX (final double x) {
		this.redPoint.setX(x);
		return this.update();
	}

	@Override
	public JPoint2D setY (final double y) {
		this.redPoint.setY(y);
		return this.update();
	}

	@Override
	public JPoint2D set (final ReadOnlyFloat2 other) {
	...
	@Override
	public JPoint2D add (final ReadOnlyFloat2 offset) {
		this.redPoint.add(offset);
		return this.update();
	}

	@Override
	public JPoint2D addX (final double delta) {
		this.redPoint.addX(delta);
		return this.update();
	}

	@Override
	public JPoint2D addY (final double delta) {
		this.redPoint.addY(delta);
		return this.update();
	}

	@Override
	public JPoint2D add (final double deltaX, final double deltaY) {
