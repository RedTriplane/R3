...
public class MotorJoint extends Joint {
  ...
  // Solver shared
  private final Vector2 m_linearOffset = new Vector2();
  private float m_angularOffset;
  private final Vector2 m_linearImpulse = new Vector2();
  ...
  private final Vector2 m_linearError = new Vector2();
  ...
  @Override
  public void getAnchorA(Vector2 out) {
    out.set(m_bodyA.getPosition());
  }

  @Override
  public void getAnchorB(Vector2 out) {
    out.set(m_bodyB.getPosition());
  }

  public void getReactionForce(float inv_dt, Vector2 out) {
   ...
   */
  public void getLinearOffset(Vector2 out) {
  ...
  public void initVelocityConstraints(SolverData data) {
    ...
    final Vector2 cA = data.positions[m_indexA].c;
    float aA = data.positions[m_indexA].a;
    final Vector2 vA = data.velocities[m_indexA].v;
    float wA = data.velocities[m_indexA].w;

    final Vector2 cB = data.positions[m_indexB].c;
    float aB = data.positions[m_indexB].a;
    final Vector2 vB = data.velocities[m_indexB].v;
    ...
    if (data.step.warmStarting) {
      ...
      final Vector2 P = m_linearImpulse;
      vA.x -= mA * P.x;
      vA.y -= mA * P.y;
