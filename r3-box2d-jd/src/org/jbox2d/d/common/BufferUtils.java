...
public class BufferUtils {
  ...
  /** Reallocate a buffer. */
  public static double[] reallocateBuffer(double[] oldBuffer, int oldCapacity, int newCapacity) {
    assert (newCapacity > oldCapacity);
  ...
  /**
   * Reallocate a double buffer. A 'deferred' buffer is reallocated only if it is not NULL. If
   * 'userSuppliedCapacity' is not zero, buffer is user supplied and must be kept.
   */
  public static double[] reallocateBuffer(double[] buffer, int userSuppliedCapacity, int oldCapacity,
      int newCapacity, boolean deferred) {
    assert (newCapacity > oldCapacity);
    assert (userSuppliedCapacity == 0 || newCapacity <= userSuppliedCapacity);
    if ((!deferred || buffer != null) && userSuppliedCapacity == 0) {
  ...
  /** Rotate an array, see std::rotate */
  public static void rotate(double[] ray, int first, int new_first, int last) {
    int next = new_first;
    while (next != first) {
      double temp = ray[first];
      ray[first] = ray[next];
      ray[next] = temp;
      first++;
      next++;
