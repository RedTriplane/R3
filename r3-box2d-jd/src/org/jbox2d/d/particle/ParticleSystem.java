...
import org.jbox2d.d.common.BufferUtils;
...
import org.jbox2d.d.dynamics.World;
import org.jbox2d.d.particle.VoronoiDiagram.VoronoiDiagramCallback;
...
public class ParticleSystem {
  ...
  Vector2[] m_accumulation2Buffer; // temporary vector values
  ...
  public ParticleSystem(World world) {
    ...
    m_positionBuffer = new ParticleBuffer<Vector2>(Vector2.class);
    m_velocityBuffer = new ParticleBuffer<Vector2>(Vector2.class);
...
//    assertNotSamePosition();
  ...
  public ParticleGroup createParticleGroup(ParticleGroupDef groupDef) {
    ...
    if (groupDef.shape != null) {
      ...
      for (double y = MathUtils.floor(aabb.lowerBound.y / stride) * stride; y < upperBoundY; y +=
          stride) {
        for (double x = MathUtils.floor(aabb.lowerBound.x / stride) * stride; x < upperBoundX; x +=
            stride) {
          Vector2 p = tempVec;
          p.x = x;
          p.y = y;
  ...
  public void computeDepthForGroup(ParticleGroup group) {
    ...
    for (int i = group.m_firstIndex; i < group.m_lastIndex; i++) {
      ...
      m_depthBuffer[i] = w < 0.8f ? 0 : Double.MAX_VALUE;
    }
...
//    assert(d2 != 0);
    if (d2 < m_squaredDiameter) {
      ...
      double invD = d2 != 0 ? MathUtils.sqrt(1 / d2) : Double.MAX_VALUE;
  ...
  public void updateBodyContacts() {
    ...
    aabb.lowerBound.x = Double.MAX_VALUE;
    aabb.lowerBound.y = Double.MAX_VALUE;
    aabb.upperBound.x = -Double.MAX_VALUE;
    aabb.upperBound.y = -Double.MAX_VALUE;
    for (int i = 0; i < m_count; i++) {
  ...
  public void solve(TimeStep step) {
    ...
    for (int i = 0; i < m_count; i++) {
      ...
      if (v2 > criticalVelocytySquared) {
        double a = v2 == 0 ? Double.MAX_VALUE : MathUtils.sqrt(criticalVelocytySquared / v2);
        v.x *= a;
        v.y *= a;
      }
    }
  ...
  void solveElastic(final TimeStep step) {
    ...
    for (int k = 0; k < m_triadCount; k++) {
      ...
      if ((triad.flags & ParticleType.b2_elasticParticle) != 0) {
        ...
        double rs = Vector2.cross(oa, pa) + Vector2.cross(ob, pb) + Vector2.cross(oc, pc);
        double rc = Vector2.dot(oa, pa) + Vector2.dot(ob, pb) + Vector2.dot(oc, pc);
        double r2 = rs * rs + rc * rc;
        double invR = r2 == 0 ? Double.MAX_VALUE : MathUtils.sqrt(1f / r2);
        rs *= invR;
        rc *= invR;
  ...
  void solveSpring(final TimeStep step) {
    ...
    for (int k = 0; k < m_pairCount; k++) {
      ...
      if ((pair.flags & ParticleType.b2_springParticle) != 0) {
        ...
        double r1 = MathUtils.sqrt(dx * dx + dy * dy);
        if (r1 == 0) r1 = Double.MAX_VALUE;
  ...
  public void raycast(ParticleRaycastCallback callback, final Vector2 point1, final Vector2 point2) {
    ...
    for (int proxy = firstProxy; proxy < lastProxy; ++proxy) {
      ...
      if (determinant >= 0) {
        double sqrtDeterminant = MathUtils.sqrt(determinant);
  ...
  static class CreateParticleGroupCallback implements VoronoiDiagramCallback {
    public void callback(int a, int b, int c) {
      ...
      if (dabx * dabx + daby * daby < maxDistanceSquared
        ...
        final double midPointx =  1 / 3 * (pa.x + pb.x + pc.x);
        final double midPointy =  1 / 3 * (pa.y + pb.y + pc.y);
            ...
            if (fixture.raycast(output, input, childIndex)) {
              ...
              p.x =
                  (1 - output.fraction) * input.p1.x + output.fraction * input.p2.x
                      + Settings.linearSlop_legacy * output.normal.x;
              p.y =
                  (1 - output.fraction) * input.p1.y + output.fraction * input.p2.y
                      + Settings.linearSlop_legacy * output.normal.y;
