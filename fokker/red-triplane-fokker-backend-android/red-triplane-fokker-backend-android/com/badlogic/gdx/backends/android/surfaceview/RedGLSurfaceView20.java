...
public class RedGLSurfaceView20 extends GLSurfaceView {
	...
	public RedGLSurfaceView20 (final Context context, final ResolutionStrategy resolutionStrategy, final int targetGLESVersion) {
		super(context);
		RedGLSurfaceView20.targetGLESVersion = targetGLESVersion;
		this.resolutionStrategy = resolutionStrategy;
		this.init(false, 16, 0);
	}

	public RedGLSurfaceView20 (final Context context, final ResolutionStrategy resolutionStrategy) {
		this(context, resolutionStrategy, 2);
	}

	public RedGLSurfaceView20 (final Context context, final boolean translucent, final int depth, final int stencil,
		final ResolutionStrategy resolutionStrategy) {
		super(context);
		this.resolutionStrategy = resolutionStrategy;
	...
	protected void onMeasure (final int widthMeasureSpec, final int heightMeasureSpec) {
		final ResolutionStrategy.MeasuredDimension measures = this.resolutionStrategy.calcMeasures(widthMeasureSpec,
			heightMeasureSpec);
	...
	public InputConnection onCreateInputConnection (final EditorInfo outAttrs) {
		...
		final BaseInputConnection connection = new BaseInputConnection(this, false) {
			@Override
			public boolean deleteSurroundingText (final int beforeLength, final int afterLength) {
				...
				if (sdkVersion >= 16) {
					...
					if (beforeLength == 1 && afterLength == 0) {
						this.sendDownUpKeyEventForBackwardCompatibility(KeyEvent.KEYCODE_DEL);
						return true;
					}
				}
	...
	private void init (final boolean translucent, final int depth, final int stencil) {
		...
		this.setEGLConfigChooser(
			translucent ? new ConfigChooser(8, 8, 8, 8, depth, stencil) : new ConfigChooser(5, 6, 5, 0, depth, stencil));
	...
	static class ContextFactory implements GLSurfaceView.EGLContextFactory {
		...
		public EGLContext createContext (final EGL10 egl, final EGLDisplay display, final EGLConfig eglConfig) {
			Log.w(TAG, "creating OpenGL ES " + RedGLSurfaceView20.targetGLESVersion + ".0 context");
			checkEglError("Before eglCreateContext " + targetGLESVersion, egl);
			final int[] attrib_list = {EGL_CONTEXT_CLIENT_VERSION, RedGLSurfaceView20.targetGLESVersion, EGL10.EGL_NONE};
			final EGLContext context = egl.eglCreateContext(display, eglConfig, EGL10.EGL_NO_CONTEXT, attrib_list);
			final boolean success = checkEglError("After eglCreateContext " + targetGLESVersion, egl);

			if ((!success || context == null) && RedGLSurfaceView20.targetGLESVersion > 2) {
				Log.w(TAG, "Falling back to GLES 2");
				RedGLSurfaceView20.targetGLESVersion = 2;
	...
	static boolean checkEglError (final String prompt, final EGL10 egl) {
		int error;
		boolean result = true;
	...
	private static class ConfigChooser implements GLSurfaceView.EGLConfigChooser {
		...
		private void printConfig (final EGL10 egl, final EGLDisplay display, final EGLConfig config) {
			final int[] attributes = {EGL10.EGL_BUFFER_SIZE, EGL10.EGL_ALPHA_SIZE, EGL10.EGL_BLUE_SIZE, EGL10.EGL_GREEN_SIZE,
